...
class IonAuthModel
{
	...
	public function clearForgottenPasswordCode(string $identity): bool
	{
		...
		return $this->db->table($this->tables['users'])->where($this->identityColumn, $identity)->orWhere('email', $identity)->orWhere('id', $identity)->update($data);
	}
	...
	public function register(string $identity, string $password, string $email, array $additionalData = [], array $groups = [])
	{
		...
		if(isset($additionalData['pegawai'])) {
			// insert pegawai first
			$this->db->table('pegawai')->insert($additionalData['pegawai']);
	...
	public function rememberUser(string $identity): bool
	{
		...
		if ($token->validatorHashed) {
			$this->db->table($this->tables['users'])->where($this->identityColumn, $identity)
				->orWhere('email', $identity)
				->update(['remember_selector' => $token->selector, 'remember_code' => $token->validatorHashed]);
	...
	public function deleteGroup(int $groupId): bool
	{
		...
		$this->setMessage('IonAuth.group_delete_successful');
		return true;
	}
	...
	public function messages(): string
	{
		...
		// check whether message is single or list
		if (sizeof($messageLang) == 1) {
			$messageLang = $messageLang[0];
			return view($this->messagesTemplates['single'], ['message' => $messageLang]);
		} else {
	...
	public function errors(string $template = 'list'): string
	{
		...
		if (sizeof($errors) == 1) {
			$errors = $errors[0];
			return view(config('Validation')->templates['single'], ['error' => $errors]);
		} else {
			return view(config('Validation')->templates['list'], ['errors' => $errors]);
		}
	}
	...
	public function getrulesImportUser()
	{
		$validationRules = [
			'excel' => [
	...
	public function import_user($file, $import)
	{
		...
		if ($file) {
			if (file_exists('assets/files/users/input.xlsx')) {
				unlink("assets/files/users/input.xlsx");
			}
			if (file_exists('assets/files/users/input.xls')) {
				unlink("assets/files/users/input.xls");
			}
			$extension = $file->getExtension();
			$file->move('assets/files/users/', 'input.' . $extension);
			$spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load("assets/files/users/{$file->getName()}");
		} else {
			if (file_exists('assets/files/users/input.xlsx')) {
				$spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load("assets/files/users/input.xlsx");
			}
			if (file_exists('assets/files/users/input.xls')) {
				$spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load("assets/files/users/input.xls");
			}
		}
		// check if the default set in config exists in database
		$query = $this->db->table($this->tables['groups'])->where(['name' => $this->config->defaultGroup], 1)->get()->getRow();
		...
		$preview = '';
		$inputDataUsers = [];
		$inputDataUsersGroups = [];

		if ($highestColumn == 'C') {
			$no = 0;

			$users = $this->db->table('users')->orderBy('id', 'desc')->get()->getRow()->id;
			$userId = !empty($users) ? $users : 0;

			for ($row = 1; $row <= $highestRow; ++$row) {

				$name = trim($sheet->getCellByColumnAndRow(1, $row)->getValue());
				$email = trim($sheet->getCellByColumnAndRow(2, $row)->getValue());
				$password = trim($sheet->getCellByColumnAndRow(3, $row)->getValue());
				$ipAddress = \Config\Services::request()->getIPAddress();
				...
				if ($row > 1) {
					...
					if ($validation->run($dataPreview, 'usersV') == FALSE) {
					...
					$errorName = $validation->hasError('name') ? "style='background: #E07171; color:black'" : '';
					$errorEmailError = $validation->hasError('email') ? "style='background: #E07171; color:black'" : '';
					$errorPassword = $validation->hasError('keterangan') ? "style='background: #E07171; color:black'" : '';

					$preview .= "
							<tr>
								<td>$no</td>
								<td $errorName>$name</td>
								<td $errorEmailError>$email</td>
								<td $errorPassword>$password</td>
							</tr>";
				}
				...
				if ($row > 1) {
					...
					// users
					array_push($inputDataUsers, array(
						'id' 				=> $userId,
						'username'          => $email,
						'email'				=> $email,
						'password'          => $this->hashPassword($password),
					...
					// users_group
					array_push($inputDataUsersGroups, array(
						'user_id' => $userId,
		...
		if (!empty($inputDataUsers) && !empty($import)) {
			$this->db->table('users')->insertBatch($inputDataUsers);
			$this->db->table('users_groups')->insertBatch($inputDataUsersGroups);
