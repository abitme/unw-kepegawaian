...
<script>
  ...
  function submitForm(method, id = null) {
    ...
    if (method == 'insert') {
      url = <?= json_encode(base_url('setting/create')) ?>;
    }
    if (method == 'update') {
      url = <?= json_encode(base_url()) ?> + `/setting/update/${id}`;
    }
      ...
      success: function(data) {
        ...
        if (obj.status == 200) {
          ...
          if (error['title']) {
            ...
            $('input[name=title]').after(`<div class="invalid-feedback">${error['title']}</div>`);
          }
          if (error['subtitle']) {
            $('input[name=subtitle]').addClass('is-invalid');
            $('input[name=subtitle]').after(`<div class="invalid-feedback">${error['subtitle']}</div>`);
          }
        }
  ...
  function createData() {
    ...
    // change text
    $('#aiModalLabel').html('Tambah Setting');
  ...
  function updateData(id) {
    ...
    // change text
    $('#aiModalLabel').html('Edit Setting');
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $('.modal-body input[name=title]').val(data.title);
        $('.modal-body input[name=subtitle]').val(data.subtitle);
        $('#aiModal').modal('show');
      },
  ...
  function destroyData(id) {
    ...
    if (x) {
      $.ajax({
        method: "POST",
        url: <?= json_encode(base_url()) ?> + `/setting/delete/${id}`,
        data: {
          id: id
        },
        cache: false,
