...
<script>
  $(document).ready(function() {
    ...
    table = $('#myTable').DataTable({
      ...
      order: [],
      ajax: {
        url: "<?= base_url('presensi-lupa-pengajuan/ajax_list') ?>",
        type: "POST",
      },
      //optional
      lengthMenu: [
  ...
  function submitForm(method, id = null) {
    ...
    if (method == 'insert') {
      url = <?= json_encode(base_url('presensi-lupa-pengajuan/create')) ?>;
    }
    if (method == 'update') {
      url = <?= json_encode(base_url()) ?> + `/presensi-lupa-pengajuan/update/${id}`;
    }
  ...
  function createData() {
    ...
    // change text
    $('#aiModalLabel').html('Tambah Pengajuan Lupa Presensi');
  ...
  function updateData(id) {
    ...
    // change text
    $('#aiModalLabel').html('Edit Pengajuan Lupa Presensi');
  ...
  function destroyData(id) {
    ...
    if (x) {
      $.ajax({
        method: "POST",
        url: <?= json_encode(base_url()) ?> + `/presensi-lupa-pengajuan/delete/${id}`,
        data: {
          id: id
        },
        cache: false,
