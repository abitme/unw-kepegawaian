...
<div class="row">
  <div class="col-md">
    <a href="javascript:void(0)" class="btn btn-primary shadow-sm mb-3" onclick="createData()">
      ...
      <span>Tambah Toleransi Terlambat</span>
    </a>
  </div>
</div>
      ...
      <div class="card-body table-responsive">
        <table class="table table-hover" id="myTable" style="overflow-x:auto;">
          <thead>
            <tr>
              <th scope="col">#</th>
              <th scope="col">Nama Toleransi</th>
              <th scope="col">Tanggal Mulai</th>
              <th scope="col">Tanggal Selesai</th>
              <th scope="col">Durasi Toleransi</th>
      ...
      <div class="modal-body">
        <form id="form" action="" method="post">
          ...
          <div class="form-group">
            <label for="nama_toleransi_terlambat">Nama Toleransi</label>
            <input type="text" class="form-control" id="nama_toleransi_terlambat" name="nama_toleransi_terlambat" />
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            <label for="durasi_toleransi">Durasi Toleransi</label>
            <input type="text" class="form-control" id="durasi_toleransi" name="durasi_toleransi" />
          </div>
...
<script>
  $(document).ready(function() {
    ...
    table = $('#myTable').DataTable({
      ...
      order: [],
      ajax: {
        url: "<?= base_url('toleransi-terlambat/ajax_list') ?>",
        type: "POST",
      },
      //optional
      lengthMenu: [
  ...
  function submitForm(method, id = null) {
    ...
    if (method == 'insert') {
      url = <?= json_encode(base_url('toleransi-terlambat/create')) ?>;
    }
    if (method == 'update') {
      url = <?= json_encode(base_url()) ?> + `/toleransi-terlambat/update/${id}`;
    }
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if (obj.status == 200) {
          ...
          if (error['nama_toleransi_terlambat']) {
            $('input[name=nama_toleransi_terlambat]').addClass('is-invalid');
            $('input[name=nama_toleransi_terlambat]').after(`<div class="invalid-feedback">${error['nama_toleransi_terlambat']}</div>`);
          }
          ...
          if (error['durasi_toleransi']) {
            $('input[name=durasi_toleransi]').addClass('is-invalid');
            $('input[name=durasi_toleransi]').after(`<div class="invalid-feedback">${error['durasi_toleransi']}</div>`);
          }
        }
  ...
  function createData() {
    ...
    // change text
    $('#aiModalLabel').html('Tambah Toleransi Terlambat');
  ...
  function updateData(id) {
    ...
    // change text
    $('#aiModalLabel').html('Edit Toleransi Terlambat');
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $('.modal-body input[name=nama_toleransi_terlambat]').val(data.nama_toleransi_terlambat);
        $('.modal-body input[name=tanggal_mulai]').val(data.tanggal_mulai);
        $('.modal-body input[name=tanggal_selesai]').val(data.tanggal_selesai);
        $('.modal-body input[name=durasi_toleransi]').val(data.durasi_toleransi);
        $('#aiModal').modal('show');
      },
  ...
  function destroyData(id) {
    ...
    if (x) {
      $.ajax({
        method: "POST",
        url: <?= json_encode(base_url()) ?> + `/toleransi-terlambat/delete/${id}`,
        data: {
          id: id
        },
        cache: false,
  ...
  $('#aiModal').on('shown.bs.modal', function() {
    $('input[name=nama_toleransi_terlambat]').trigger('focus')
  })
