...
<script>
  ...
  function submitForm(method, id = null) {
    ...
    if (method == 'insert') {
      url = <?= json_encode(base_url('jabatan/create')) ?>;
    }
    if (method == 'update') {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if (obj.status == 200) {
          ...
          if (error['nama_jabatan']) {
            $('input[name=nama_jabatan]').addClass('is-invalid');
            $('input[name=nama_jabatan]').after(`<div class="invalid-feedback">${error['nama_jabatan']}</div>`);
          }
        }
  ...
  function updateData(id) {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $('.modal-body input[name=nama_jabatan]').val(data.nama_jabatan);
        $('#aiModal').modal('show');
      },
  ...
  function destroyData(id) {
    ...
    if (x) {
      $.ajax({
        method: "POST",
        url: <?= json_encode(base_url()) ?> + `/jabatan/delete/${id}`,
        data: {
          id: id
        },
        cache: false,
  ...
  $('#aiModal').on('shown.bs.modal', function() {
    $('input[name=nama_jabatan]').trigger('focus')
  })
