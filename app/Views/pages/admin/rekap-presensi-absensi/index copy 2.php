      ...
      <div class="card-body table-responsive">
        ...
        <table class="table table-hover" id="myTable" style="overflow-x:auto;">
          ...
          <tbody>
            ...
            // $toleransiTerlambat = $db->query("SELECT * FROM `toleransi_terlambat`")->getResult();
            ...
            foreach ($pegawai as $p) : $no++ ?>
              ...
              // $terlambat = 0;
              // foreach ($toleransiTerlambat as $item) {
              //   $toleransiTanggalAwal = $db->query("SELECT durasi_toleransi FROM `toleransi_terlambat` WHERE $tanggal_awal BETWEEN tanggal_mulai AND CASE WHEN tanggal_selesai IS NULL THEN CURDATE() + INTERVAL 1 YEAR ELSE tanggal_selesai END")->getRow();
              //   if ($toleransiTanggalAwal) {
              //     $terlambat += $db->query("SELECT COUNT(*) as terlambat FROM view_rekap_presensi WHERE id_pegawai = $p->id AND SUBSTRING_INDEX(SUBSTRING_INDEX(jam_masuk_pulang, ' - ', 1), ' - ', -1) > ADDTIME(SUBSTRING_INDEX(SUBSTRING_INDEX(jadwal_jam_masuk_pulang, ' - ', 1), ' - ', -1), $toleransiTanggalAwal->durasi_toleransi) AND tanggal BETWEEN '$tanggal_awal' AND '$toleransiTanggalAwal->tanggal_selesai'")->getRow()->terlambat;
              //   }
              //   $toleransiTanggalAkhir = $db->query("SELECT durasi_toleransi FROM `toleransi_terlambat` WHERE $tanggal_akhir BETWEEN tanggal_mulai AND CASE WHEN tanggal_selesai IS NULL THEN CURDATE() + INTERVAL 1 YEAR ELSE tanggal_selesai END")->getRow();
              //   if ($toleransiTanggalAkhir) {
              //     $terlambat += $db->query("SELECT COUNT(*) as terlambat FROM view_rekap_presensi WHERE id_pegawai = $p->id AND SUBSTRING_INDEX(SUBSTRING_INDEX(jam_masuk_pulang, ' - ', 1), ' - ', -1) > ADDTIME(SUBSTRING_INDEX(SUBSTRING_INDEX(jadwal_jam_masuk_pulang, ' - ', 1), ' - ', -1), $toleransiTanggalAwal->durasi_toleransi) AND tanggal BETWEEN '$toleransiTanggalAwal->tanggal_mulai' AND '$tanggal_akhir'")->getRow()->terlambat;
              //   }
              // }
              ...
              if ($isBetweenDifferentRule) {
                ...
                if ($jabatanUser && $jabatanUser->nama_jabatan == 'Dosen' && $tanggal_awal >= $konfigurasiPresensi->tanggal_mulai_presensi_dosen_sekali) {
                  ...
                  // durasi jam kerja biasa
                  $durasiJamKerja = $db->query("SELECT SUM(TIME_TO_SEC(jam_timediff)/3600) as total_hours FROM view_rekap_presensi WHERE id_pegawai = $p->id AND (alasan_pulang_cepat != 'Mengajar' OR  alasan_pulang_cepat IS NULL) AND tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' $notInLupaPresensi $notInIzin")->getRow()->total_hours ?? '0';
                  // durasi jam kerja full sesuai jadwal jam kerja karena mengajar kelas regulaer diluar jam kerja
                  $durasiJamKerjaMengajar = $db->query("SELECT SUM(TIME_TO_SEC(jadwal_timediff)/3600) as total_hours FROM view_rekap_presensi WHERE id_pegawai = $p->id AND alasan_pulang_cepat = 'Mengajar' AND tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' $notInLupaPresensi $notInIzin")->getRow()->total_hours ?? '0';
                  // durasi ditambah 1 jam karena lupa absen pulang 
                  $durasiJamKerjaPlus1 = $db->query("SELECT SUM(TIME_TO_SEC('01:00:00.000000')/3600) as total_hours FROM view_rekap_presensi WHERE id_pegawai = $p->id AND jam_timediff = '00:00:00.000000' AND tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' $notInLupaPresensi $notInIzin")->getRow()->total_hours ?? '0';
                  // durasi ditambah durasi lupa presensi yang tervalidasi
