...
<div class="row">
  <div class="col-md">
    ...
    <a href="<?= base_url('jabatan-fungsional/import') ?>" class="btn btn-success shadow-sm mb-3">
      <i class="fas fa-plus-circle"></i>
      ...
      <div class="modal-body">
        <form id="form" action="" method="post">
          ...
          <div class="form-group">
            ...
            <?= form_dropdown('id_jabatan', getDropdownList('jabatan', ['id', 'nama_jabatan'], '', '- Pilih jabatan -', 'nama_jabatan', 'asc', ''), set_value('id_jabatan'), ['class' => 'form-control select2-jabatan', 'id' => 'id_jabatan', 'style' => 'width:100%']) ?>
          </div>
          <div class="form-group">
            <label for="nama_jabatan_fungsional">Nama Jabatan Fungsional</label>
            <input type="text" class="form-control" id="nama_jabatan_fungsional" name="nama_jabatan_fungsional" autocomplete="off">
          </div>
...
<script>
  $(document).ready(function() {
    ...
    table = $('#myTable').DataTable({
      ...
      order: [],
      ajax: {
        url: "<?= base_url('jabatan-fungsional/ajax_list') ?>",
        type: "POST",
      },
      //optional
      lengthMenu: [
    ...
    $('.select2-jabatan').select2({
      ajax: {
        url: "<?= base_url('jabatan/ajax_select2') ?>",
        type: "post",
        dataType: 'json',
        delay: 250,
  ...
  function submitForm(method, id = null) {
    ...
    if (method == 'insert') {
      url = <?= json_encode(base_url('jabatan-fungsional/create')) ?>;
    }
    if (method == 'update') {
      url = <?= json_encode(base_url()) ?> + `/jabatan-fungsional/update/${id}`;
    }
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if (obj.status == 200) {
          ...
          if (error['id_jabatan']) {
            $('select[name=id_jabatan] + .select2 .select2-selection--single').addClass('is-invalid');
            $('select[name=id_jabatan] + .select2 .select2-selection--single').after(`<div class="invalid-feedback">${error['id_jabatan']}</div>`);
          }
          if (error['nama_jabatan_fungsional']) {
            $('input[name=nama_jabatan_fungsional]').addClass('is-invalid');
            $('input[name=nama_jabatan_fungsional]').after(`<div class="invalid-feedback">${error['nama_jabatan_fungsional']}</div>`);
          }
        }
  ...
  function createData() {
    ...
    $('#form')[0].reset();
    $('#id').val('');
    $('span#select2-id_jabatan-container').html('-Pilih jabatan-');
  ...
  function updateData(id) {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $('.modal-body select[name=id_jabatan]').val(data.id_jabatan).trigger('change');
        $('span#select2-id_jabatan-container').html(data.nama_jabatan);
        $('.modal-body input[name=nama_jabatan_fungsional]').val(data.nama_jabatan_fungsional);
  ...
  function destroyData(id) {
    ...
    if (x) {
      $.ajax({
        method: "POST",
        url: <?= json_encode(base_url()) ?> + `/jabatan-fungsional/delete/${id}`,
        data: {
          id: id
        },
        cache: false,
