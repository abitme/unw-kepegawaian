...
<div class="row">
  <div class="col-md">
    ...
    <a href="<?= base_url('jabatan-unit/import') ?>" class="btn btn-success shadow-sm mb-3">
      <i class="fas fa-plus-circle"></i>
...
<script>
  $(document).ready(function() {
    ...
    table = $('#myTable').DataTable({
      ...
      order: [],
      ajax: {
        url: "<?= base_url('jabatan-unit/ajax_list') ?>",
        type: "POST",
      },
      //optional
      lengthMenu: [
  ...
  function submitForm(method, id = null) {
    ...
    if (method == 'insert') {
      url = <?= json_encode(base_url('jabatan-unit/create')) ?>;
    }
    if (method == 'update') {
      url = <?= json_encode(base_url()) ?> + `/jabatan-unit/update/${id}`;
    }
  ...
  function updateData(id) {
    ...
    $.ajax({
      method: 'POST',
      url: <?= json_encode(base_url()) ?> + `/jabatan/update/${id}`,
      cache: false,
      success: function(data) {
  ...
  function destroyData(id) {
    ...
    if (x) {
      $.ajax({
        method: "POST",
        url: <?= json_encode(base_url()) ?> + `/jabatan-unit/delete/${id}`,
        data: {
          id: id
        },
        cache: false,
