    ...
    <div class="modal-content">
      ...
      <form id="form" action="" method="post">
        <div class="modal-body">
          ...
          <div class="form-group">
            <label for="id_pertanyaan_kategori">Kategori</label>
            <?= form_dropdown('id_pertanyaan_kategori', getDropdownList('pertanyaan_kategori', ['id', 'kategori'], '', '- Pilih Kategori -', 'id', 'asc', ''), set_value('id_pertanyaan_kategori'), ['class' => 'form-control select2-pertanyaan-kategori', 'id' => 'id_pertanyaan_kategori', 'style' => 'width:100%']) ?>
          </div>
        </div>
...
<script>
  $(document).ready(function() {
    table = $('#myTable').DataTable({
      ...
      order: [],
      ajax: {
        url: "<?= base_url('pertanyaan/ajax_list') ?>",
        type: "POST"
      },
      //optional
      // lengthMenu: [
    ...
    // select2 pertanyaan-kategori
    $('.select2-pertanyaan-kategori').select2({
      ajax: {
        url: "<?= base_url('pertanyaan-kategori/ajax_select2') ?>",
        type: "post",
        dataType: 'json',
        delay: 250,
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if (obj.status == 200) {
          ...
          let error = obj.data;
          if (error['id_pertanyaan_kategori']) {
            $('select[name=id_pertanyaan_kategori] + .select2 .select2-selection--single').addClass('is-invalid');
            $('select[name=id_pertanyaan_kategori] + .select2 .select2-selection--single').after(`<div class="invalid-feedback">${error['id_pertanyaan_kategori']}</div>`);
          }
          if (error['pertanyaan']) {
  ...
  function createData() {
    ...
    $('#form')[0].reset();
    $('#id').val('');
    $('span#select2-id_pertanyaan_kategori-container').html('-Pilih Kategori-');
  ...
  function updateData(id) {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $('.modal-body select[name=id_pertanyaan_kategori]').val(data.id_pertanyaan_kategori).trigger('change');
        $('span#select2-id_pertanyaan_kategori-container').html(data.kategori);
