...
<div class="row">
  <div class="col-md">
    <a href="javascript:void(0)" class="btn btn-primary shadow-sm mb-3" onclick="createDataUnit()">
...
<!-- Modal Unit -->
<div class="modal fade" id="aiModalUnit" tabindex="-1" role="dialog" aria-labelledby="aiModalUnitLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="aiModalUnitLabel">...</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form id="form-unit" action="" method="post">
          <input type="hidden" name="id" id="id">
          <div class="form-group">
            <label for="nama_unit">Nama Unit</label>
            <input type="text" class="form-control" id="nama_unit" name="nama_unit" autocomplete="off">
          </div>
          ...
          <div class="form-check">
            <input type="checkbox" class="form-check-input" id="is_separated" value="1" name="is_separated">
            <label class="form-check-label is_separated" for="is_separated">
              Is Separated
            </label>
          </div>
          <div class="form-check">
            <input type="checkbox" class="form-check-input" id="is_assess" value="1" name="is_assess">
            <label class="form-check-label is_assess" for="is_assess">
              Is Assess
            </label>
          </div>
          <div class="form-check">
            <input type="checkbox" class="form-check-input" id="is_picket" value="1" name="is_picket">
            <label class="form-check-label is_picket" for="is_picket">
              Is Picket
            </label>
          </div>
...
<script>
  ...
  function submitFormUnit(method, id = null) {

    // remove form error message
    $('.form-text').remove();

    if (method == 'insert') {
      url = <?= json_encode(base_url('unit/create')) ?>;
    }
    if (method == 'update') {
      url = <?= json_encode(base_url()) ?> + `/unit/update/${id}`;
    }

  }

  // Unit - handle submit form
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if (obj.status == 200) {
          ...
          if (error['nama_unit']) {
            $('input[name=nama_unit]').addClass('is-invalid');
            $('input[name=nama_unit]').after(`<div class="invalid-feedback">${error['nama_unit']}</div>`);
          }
        }
  ...
  function createDataUnit() {
    ...
    $('#is_separated').prop('checked', true);
    $('#is_assess').prop('checked', true);
    $('#is_picket').prop('checked', true);

    // change text
    $('#aiModalUnitLabel').html('Tambah Unit');
    $('.modal-footer button[type=submit]').html('Create');

    $('#aiModalUnit').modal('show');

    submitFormUnit('insert');
  }

  // Unit - ajax Update
  function updateDataUnit(id) {
    removeValidation()
    $('#form-unit')[0].reset();

    // change text
    $('#aiModalUnitLabel').html('Edit Unit');
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $('.modal-body input[name=nama_unit]').val(data.nama_unit);
        $('.modal-body input[name=is_active]').prop("checked", data.is_active == true);
        $('.modal-body input[name=is_separated]').prop("checked", data.is_separated == true);
        $('.modal-body input[name=is_assess]').prop("checked", data.is_assess == true);
        $('.modal-body input[name=is_picket]').prop("checked", data.is_picket == true);
        $('#aiModalUnit').modal('show');
      },
    ...
    submitFormUnit('update', id);
  }

  // Unit - ajax Delete
  function destroyDataUnit(id) {
    var x = confirm('Apakah anda yakin menghapus data?');
    if (x) {
      $.ajax({
        method: "POST",
        url: <?= json_encode(base_url()) ?> + `/unit/delete/${id}`,
        data: {
          id: id
        },
        cache: false,
  ...
  // Unit - set focus on shown modal 
  $('#aiModalUnit').on('shown.bs.modal', function() {
