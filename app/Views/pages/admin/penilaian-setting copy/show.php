  ...
  <div class="col-md-12">
    <div class="card shadow mb-3">
      <div class="card-header text-center">
        <h6 class="m-0 font-weight-bold text-primary">Pengaturan Penilaian (<?= date('d/m/Y', strtotime($periode->tanggal_awal)) ?> - <?= date('d/m/Y', strtotime($periode->tanggal_akhir)) ?>)</h6>
      </div>
      <div class="card-body table-responsive">

        <form action="<?= base_url("penilaian-setting/$periode->id/upsert") ?>" method="post">
          <?= csrf_field() ?>
          ...
          <div class="row">
            <div class="col-6">
              <table class="table table-borderless mb-0">
                <!-- <tr class="d-flex">
                  <td class="col-5">Periode</td>
                  <td><?= date('d/m/Y', strtotime($periode->tanggal_awal)) ?> - <?= date('d/m/Y', strtotime($periode->tanggal_akhir)) ?></td>
                </tr> -->
                <tr class="d-flex">
                  <td class="col-5">Periode Penilaian</td>
                  <td><input type="text" name="periode_penilaian" class="form-control" value="<?= set_value('periode_penilaian', $penilaian_setting->periode_penilaian ?? '') ?>"></td>
                </tr>
                <tr class="d-flex">
                  <td class="col-5">Tanggal Mulai Penilaian</td>
                  <td><input type="date" name="tanggal_mulai" class="form-control" value="<?= set_value('tanggal_mulai', $penilaian_setting->tanggal_mulai ?? '') ?>"></td>
                </tr>
                <tr class="d-flex">
                  <td class="col-5">Tanggal Selesai Penilaian</td>
                  <td><input type="date" name="tanggal_selesai" class="form-control" value="<?= set_value('tanggal_selesai', $penilaian_setting->tanggal_selesai ?? '') ?>"></td>
                </tr>
              </table>
            </div>
            <div class="col-5 offset-1">
              <table class="table table-borderless mb-0">
                <!-- <tr class="d-flex">
                <td class="col-5">Tanggal Selesai</td>
                <td>: &nbsp; <?= $penilaian_setting->tanggal_selesai ?? ''  ?></td>
              </tr> -->
              </table>
            </div>
          </div>
              ...
              <table class="table table-input mb-4" id="table">
                ...
                <tbody class="container-ai">
                  ...
                  <?php else : ?>
                    <?php
                    $no = 0;
                    foreach ($pertanyaan_periode as $row) : $no++; ?>
                      <tr class="row-input-ai">
                        <input class="form-control" type="hidden" name="id_pertanyaan[]" value="<?= $row->id_pertanyaan ?>">
                        <td>
                          <input class="form-control" type="hidden" name="pertanyaan[]" value="<?= $row->pertanyaan ?>">
                          <?= $row->pertanyaan ?>
                        </td>
                        <td>
                          <input class="form-control currency<?= $no ?> no_urut" type="text" name="no_urut[]" value="<?= $row->no_urut ?>" autocomplete="off" style="width:100px">
                        </td>
                        <td>
                          <a href="#" class="delete-input">Hapus</a>
                        </td>
