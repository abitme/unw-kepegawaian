...
<script>
  $(document).ready(function() {
    ...
    table = $('#myTable').DataTable({
      ...
      order: [],
      ajax: {
        url: "<?= base_url('presensi-lupa-validasi/ajax_list') ?>",
        type: "POST",
      },
      //optional
      lengthMenu: [
  ...
  function submitForm(method, id = null) {
    ...
    if (method == 'insert') {
      url = <?= json_encode(base_url('presensi-lupa-validasi/create')) ?>;
    }
    if (method == 'update') {
      url = <?= json_encode(base_url()) ?> + `/presensi-lupa-validasi/update/${id}`;
    }
      ...
      success: function(data) {
        ...
        if (obj.status == 200) {
          ...
          if (error['status']) {
            ...
            $('select[name=status]').after(`<div class="invalid-feedback">${error['status']}</div>`);
          }
        }
  ...
  function updateData(id) {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $('.modal-body input[name=id_pegawai]').val(data.id_pegawai);
        $('.modal-body select[name=status]').val(data.status);
