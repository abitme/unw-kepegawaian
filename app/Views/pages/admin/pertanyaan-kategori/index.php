      ...
      <div class="modal-body">
        <form id="form" action="" method="post">
          ...
          <div class="form-group">
            ...
            <input type="number" class="form-control" id="nilai_min" name="nilai_min" autocomplete="off">
          </div>
          <div class="form-group">
            <label for="nilai_max">Nilai Max</label>
            <input type="number" class="form-control" id="nilai_max" name="nilai_max" autocomplete="off">
          </div>
          <div class="form-group">
            <label for="range_desc">Range Desc</label>
            <input type="text" class="form-control" id="range_desc" name="range_desc" autocomplete="off">
          </div>
...
<script>
  $(document).ready(function() {
    ...
    table = $('#myTable').DataTable({
      ...
      order: [],
      ajax: {
        url: "<?= base_url('pertanyaan-kategori/ajax_list') ?>",
        type: "POST",
      },
      //optional
      lengthMenu: [
  ...
  function submitForm(method, id = null) {
    ...
    if (method == 'insert') {
      url = <?= json_encode(base_url('pertanyaan-kategori/create')) ?>;
    }
    if (method == 'update') {
      url = <?= json_encode(base_url()) ?> + `/pertanyaan-kategori/update/${id}`;
    }
      ...
      success: function(data) {
        ...
        if (obj.status == 200) {
          ...
          if (error['kategori']) {
            ...
            $('input[name=kategori]').after(`<div class="invalid-feedback">${error['kategori']}</div>`);
          }
          if (error['nilai_min']) {
            $('input[name=nilai_min]').addClass('is-invalid');
            $('input[name=nilai_min]').after(`<div class="invalid-feedback">${error['nilai_min']}</div>`);
          }
          if (error['nilai_max']) {
            $('input[name=nilai_max]').addClass('is-invalid');
            $('input[name=nilai_max]').after(`<div class="invalid-feedback">${error['nilai_max']}</div>`);
          }
          if (error['range_desc']) {
            $('input[name=range_desc]').addClass('is-invalid');
            $('input[name=range_desc]').after(`<div class="invalid-feedback">${error['range_desc']}</div>`);
          }
        }
  ...
  function createData() {
    ...
    // change text
    $('#aiModalLabel').html('Tambah Kategori');
  ...
  function updateData(id) {
    ...
    // change text
    $('#aiModalLabel').html('Edit Kategori');
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $('.modal-body input[name=kategori]').val(data.kategori);
        $('.modal-body input[name=nilai_min]').val(data.nilai_min);
        $('.modal-body input[name=nilai_max]').val(data.nilai_max);
        $('.modal-body input[name=range_desc]').val(data.range_desc);
        $('#aiModal').modal('show');
      },
  ...
  function destroyData(id) {
    ...
    if (x) {
      $.ajax({
        method: "POST",
        url: <?= json_encode(base_url()) ?> + `/pertanyaan-kategori/delete/${id}`,
        data: {
          id: id
        },
        cache: false,
  ...
  $('#aiModal').on('shown.bs.modal', function() {
    $('input[name=kategori]').trigger('focus')
  })
