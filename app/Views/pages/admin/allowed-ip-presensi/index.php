...
<script>
  $(document).ready(function() {
    ...
    table = $('#myTable').DataTable({
      ...
      order: [],
      ajax: {
        url: "<?= base_url('allowed-ip-presensi/ajax_list') ?>",
        type: "POST",
      },
      //optional
      lengthMenu: [
  ...
  function submitForm(method, id = null) {
    ...
    if (method == 'insert') {
      url = <?= json_encode(base_url('allowed-ip-presensi/create')) ?>;
    }
    if (method == 'update') {
      url = <?= json_encode(base_url()) ?> + `/allowed-ip-presensi/update/${id}`;
    }
      ...
      success: function(data) {
        ...
        if (obj.status == 200) {
          ...
          if (error['ip_address']) {
            ...
            $('input[name=ip_address]').after(`<div class="invalid-feedback">${error['ip_address']}</div>`);
          }
        }
  ...
  function updateData(id) {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $('.modal-body input[name=ip_address]').val(data.ip_address);
        $('#aiModal').modal('show');
      },
  ...
  function destroyData(id) {
    ...
    if (x) {
      $.ajax({
        method: "POST",
        url: <?= json_encode(base_url()) ?> + `/allowed-ip-presensi/delete/${id}`,
        data: {
          id: id
        },
        cache: false,
