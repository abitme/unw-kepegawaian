      ...
      <div class="modal-body">
        <form id="form" action="" method="post">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="nama_acara" name="nama_acara" autocomplete="off">
          </div>
          <div class="form-group">
...
<script>
  ...
  function submitForm(method, id = null) {
    ...
    if (method == 'insert') {
      url = <?= json_encode(base_url('acara/create')) ?>;
    }
    if (method == 'update') {
      url = <?= json_encode(base_url()) ?> + `/acara/update/${id}`;
    }
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if (obj.status == 200) {
          ...
          if (error['nama_acara']) {
            $('input[name=nama_acara]').addClass('is-invalid');
            $('input[name=nama_acara]').after(`<div class="invalid-feedback">${error['nama_acara']}</div>`);
          }
          if (error['tanggal']) {
  ...
  function createData() {
    ...
    // change text
    $('#aiModalLabel').html('Tambah Acara');
  ...
  function updateData(id) {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $('.modal-body input[name=nama_acara]').val(data.nama_acara);
  ...
  function destroyData(id) {
    ...
    if (x) {
      $.ajax({
        method: "POST",
        url: <?= json_encode(base_url()) ?> + `/acara/delete/${id}`,
        data: {
          id: id
        },
        cache: false,
  ...
  $('#aiModal').on('shown.bs.modal', function() {
    $('input[name=nama_acara]').trigger('focus')
  })
